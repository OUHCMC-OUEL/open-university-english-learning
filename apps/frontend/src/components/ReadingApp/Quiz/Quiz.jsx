...
function Quiz({ passage, questions, index, setIndex, setPartHistory}) {
    ...
    const { submitQuiz,isSubmitting } = useQuizSubmit({questions, userAnswers,passage, setPartHistory, startTime});
            ...
            <div className="flex-1 flex flex-col gap-4 w-full">
                <Card className="w-full">
                    <CardHeader>
                        ...
                        <CardDescription>{questions[index].question_text}</CardDescription>
                        <CardAction>
                            <QuestionSheet
                                questions={questions}
                                userAnswers={userAnswers}
                ...
                {!used[index] && (
                    <Button onClick={() => apply5050(index, questions[index].correct_answer)}>
                        50 / 50
                    </Button>
                )}
            </div>
